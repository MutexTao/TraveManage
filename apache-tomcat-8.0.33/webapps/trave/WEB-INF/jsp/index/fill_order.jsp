...
<script>
    function headFileUploadFunction(){
        ...
        formData.append("uploadFile",$("#headFileUpload").prop("files")[0]);
        $.ajax({
            url:"http://localhost:8088/pic/upload1",
            type:"post",
            data: formData,
            ...
            success: function(data){
                ...
                if ($("#img").length>0){
                    ...
                    $("#addimg").append("<img id='img' src='"+data.data+"'/>");
                }
                $("#healthcodeimage").val(data.data);
            }
        });
    }
</script>
<script>
    $(".theme_r_btn").click(function(){
        ...
        var healthcodeimage=$("input[name='healthcodeimage']").val();
        if (username==""){
            alert("请填写联系人");
            return ;
        }
        ...
        $.ajax({
                ...
                success : function(result) {//返回数据根据结果进行相应的处理
                    if ( result.success ) {
                        alert('购买成功');
                        window.location.href="http://localhost:8088/productOrder/orderView";
                    } else {
                        alert(result.msg);
                    }
                }
