...
<script>
    function init(){
        console.log(${pid});
        $.ajax({
            url:"http://localhost:8088/manage/productSell/list/"+${pid},
            async: false,
            type:"get",
            dataType:"json",
                        ...
                        $.ajax({
                            ...
                            success:function(result){
                                ...
                                $.each(data.records,function(i,item){
                                    var edtior="<td class='center'><a href='http://localhost:8088/manage/productSell/updateView/"+item.id+"' title='编辑' class='link_icon' >&#101;</a><a  title='删除' class='link_icon'value='"+item.id+"' onclick='del(this)'>&#100;</a></td>";
                                    showTableHtml += "<tr>"+
    ...
    function del(obj) {
        ...
        if(confirm("确实要删除吗？")){
            $.ajax({
                type:"GET",
                url:"http://localhost:8088/manage/productSell/delete/"+id,
                dataType:"json",
                success:function(obj){
                    if(obj.status==200){
                        alert("删除成功");
                        window.location.href="http://localhost:8088/manage/productSell/listView/"+${pid};
                    }
