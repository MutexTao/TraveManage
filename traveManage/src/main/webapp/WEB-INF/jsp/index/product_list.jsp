                        ...
                        $.ajax({
                            ...
                            success:function(result){
                                ...
                                $.each(data.records,function(i,item){
                                    ...
                                    var tpUrl="http://localhost:8088/themeProduct/list/"+item.pid;
                                    var tpList;
                                    $.ajax({
                                        url : tpUrl,
                                        type : 'get',
...
//                                        console.log(value);
                                    ...
                                    showTableHtml +=
                                        ...
                                        "<a href='http://localhost:8088/product/detailView/"+item.pid+"'>"+
                                        ...
                                        "<a href='http://localhost:8088/product/detailView/"+item.pid+"'>立即预订</a>"+
                                        "</div>"+
                                        "</div>";
                                });
    ...
    $(function() {
        ...
        var title = arr[0].split('=')[1];//获取数组中第一个参数

        var theme = arr[1].split('=')[1];//第二个参数

        title =decodeURI(title);//转码将解码方式unscape换为decodeURI，将中文参数获取
        theme =decodeURI(theme);
        var data_title = 'startAreaname=&endAreaname=&title='+title+'&status=1&activeDays=不限&type='+theme;
        subData(data_title);

    });

    function subData(data) {
        console.log('%c提交数据:'+data,'color:blue');
        $.ajax({
            url:"http://localhost:8088/product/list",
            async: false,
            type:"POST",
