<%@ page import="com.halfsummer.common.Const" %>
...
<style>
    ...
    .fc-past .fc-day-number, .fc-today .fc-day-number {
        color: #ccc;
    }
                    ...
                    <div class="info_desc_txt">
                        ...
                        <ul class="info_list_attr">
                            ...
                            <li>
                                ...
                                <span class="info_attr_val"><input id="pnumber" name="pnumber" type="number" min="1"
                                                                   value="1"></span>
                            </li>
                            <li>
                                <span class="info_attr_txt">房型选择</span>
                                <span class="info_attr_val">
                                    <select id="houseType" name="houseType" type="text"
...
<script type="text/javascript">
    $('.calendar-box').calendar({
        ...
        data: [
            ...
            {
                date: '<fmt:formatDate value="${item.date}" pattern="yyyy-MM-dd"/>',
                data: '${item.price}',
                psid: '${item.psid}'
            },
    ...
    function getActive() {
        ...
        var orderdate = data.date.toString().split('-');
        var tempdata = new Date();
        var localdate = tempdata.toLocaleDateString().split('/');
        if (parseInt(orderdate[0]) < parseInt(localdate[0])) {
            alert("该日期已过了");
            return;
        } else {
            if (parseInt(orderdate[1]) < parseInt(localdate[1])) {
                alert("该日期已过了");
                return;
            } else {
                if (parseInt(orderdate[1]) == parseInt(localdate[1]) && parseInt(orderdate[2]) < parseInt(localdate[2])) {
                    alert("该日期已过了");
                    return;
                } else {
                }
        ...
        var user = '<%=session.getAttribute(Const.CURRENT_USER) %>';
        ...
        if (confirm(str) == true) {
            if(user == "null")
                ...
                var url = "http://localhost:8088/productOrder/fillView?" + "psid=" + data.psid + "&num=" + numbers;
                window.location.href = url;
            }

//            $.ajax({
//                type: 'POST',
//                url: "http://localhost:8088/productOrder/create",
//                data : {
//                    "id" : data.psid,
//                },
...
//            });
    ...
    function chooseHouseType() {
        // var data = $('.calendar-box').calendarGetActive();
        var houseType = $('#houseType').val();

        $.ajax({
            type: 'POST',
            url: "http://localhost:8088/product/getDate",
            data: {
